...
mkv = MarkovChain()
mkv.bulk_train(r'C:\Users\Hoku\Desktop\stored_data\*.txt', verbose=True)

# Store this information for later, so that there's no need to re-train the next time.
mkv.save_training(r'C:\Users\Hoku\Desktop\stored data training')
...
class Music:
    ...
    async def speak(self, ctx):
        ...
        if state.voice is None:
            ...
            if not success:
                return
        try:
            player = state.voice.create_ffmpeg_player('123.mp3')
        except Exception as e:
            fmt = 'An error occurred while processing this request: ```py\n{}: {}\n```'
            await self.bot.send_message(ctx.message.channel, fmt.format(type(e).__name__, e))
        else:
    ...
    async def on_ready(self):
        await self.bot.say('Logged in as', bot.user.name)

    @commands.command(pass_context=True, no_pm=True)
    async def hello(self):
        await self.bot.say('Hello, darling')
